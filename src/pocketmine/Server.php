...
//use pocketmine\item\enchantment\EnchantmentLevelTable; isLevelGenerated
...
class Server{
	...
	public $dserverPlayers = 0; //dserver
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath, $defaultLang = "unknown"){
		...
		try{
			...
			if(file_exists($this->filePath . "src/pocketmine/resources/Rocky_$lang.yml")){
				$content = file_get_contents($file = $this->filePath . "src/pocketmine/resources/Rocky_$lang.yml");
			}else{
				$content = file_get_contents($file = $this->filePath . "src/pocketmine/resources/Rocky_eng.yml");
			}

			if(!file_exists($this->dataPath . "Rocky.yml")){
				@file_put_contents($this->dataPath . "Rocky.yml", $content);
			}
			$internelConfig = new Config($file, Config::YAML, []);
			$this->advancedConfig = new Config($this->dataPath . "Rocky.yml", Config::YAML, []);
...
//properties
		...
		if($this->netherEnabled){
			if(!$this->loadLevel($this->netherName)){
				$this->logger->info("nether loading".$this->netherName);
			...
			if($cfgVer > $advVer){
				$this->logger->notice("Your blackmine.yml needs update");
